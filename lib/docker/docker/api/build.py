...
class BuildApiMixin(object):
  def build(self,
    ...
    """
        ...
        Args:
            ...
            cache_from (:py:class:`list`): A list of images used for build cache
              resolution
            target (str): Name of the build-stage to build in a multi-stage
              Dockerfile
            network_mode (str): networking mode for the run commands during
              build
            squash (bool): Squash the resulting images layers into a single
              layer.
  ...
  def _set_auth_headers(self, headers):
    ...
    if self._auth_configs:
      ...
      log.debug('Sending auth config ({0})'.format(', '.join(
          repr(k) for k in auth_data.keys())))
...
def process_dockerfile(dockerfile, path):
  ...
  if (os.path.splitdrive(path)[0] != os.path.splitdrive(abs_dockerfile)[0] or
    ...
    with open(abs_dockerfile, 'r') as df:
      return ('.dockerfile.{0:x}'.format(random.getrandbits(160)), df.read())
