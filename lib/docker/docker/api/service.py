    ...
    """
        ...
        Args:
            ...
            labels (dict): A map of labels to associate with the service.
              Optional.
            mode (ServiceMode): Scheduling mode for the service (replicated or
  ...
  def service_logs(self,
    ...
    """
            ...
            Args:
                ...
                tail (string or int): Number of log lines to be returned,
                  counting from the current end of the logs. Specify an integer
                  or ``'all'`` to output all log lines.
                    Default: ``all``
                is_tty (bool): Whether the service's :py:class:`ContainerSpec`
                  enables the TTY option. If omitted, the method will query the
                  Engine for the information, causing an additional roundtrip.
    ...
    if is_tty is None:
      is_tty = self.inspect_service(
          service)['Spec']['TaskTemplate']['ContainerSpec'].get('TTY', False)
  ...
  def update_service(self,
    ...
    if fetch_current_spec:
      ...
      current = self.inspect_service(
          service, insert_defaults=inspect_defaults)['Spec']
