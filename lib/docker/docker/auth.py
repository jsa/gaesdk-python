...
def split_repo_name(repo_name):
  ...
  if len(parts) == 1 or ('.' not in parts[0] and ':' not in parts[0] and
                         parts[0] != 'localhost'):
...
class AuthConfig(dict):
  ...
  def parse_auth(cls, entries, raise_on_error=False):
    ...
    for registry, entry in six.iteritems(entries):
      ...
      if 'auth' not in entry:
        ...
        log.debug('Auth data for {0} is absent. Client might be using a '
                  'credentials store instead.'.format(registry))
        conf[registry] = {}
        continue

      username, password = decode_auth(entry['auth'])
      log.debug('Found entry (registry={0}, username={1})'.format(
  ...
  def load_config(cls, config_path, config_dict, credstore_env=None):
    ...
    log.debug("Couldn't find auth-related section ; attempting to interpret "
              'as auth-only file')
  ...
  def _resolve_authconfig_credstore(self, registry, credstore_name):
    ...
    try:
      ...
      raise errors.DockerException('Credentials store error: {0}'.format(
          repr(e)))
